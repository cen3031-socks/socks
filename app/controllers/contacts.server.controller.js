...
exports.determineIfEmployee = function (contact) {
	var deferred = q.defer();
	console.log(contact._id);
	User.find({contact: contact._id, permissionLevel: users.EMPLOYEE}).
...
exports.findAdmins = function (req, res) {
	User.find({contact: req.contact._id, permissionLevel: users.ADMIN}).
		exec(function (err, users) {
			if (err) {
				return res.status(400);
			}
...
exports.determineIfAdmin = function (contact) {
	var deferred = q.defer();
	console.log(contact._id);
	User.find({contact: contact._id, permissionLevel: users.ADMIN}).
		exec(function (err, users) {
			if (err) {
				deferred.reject();
			}
...
exports.update = function (req, res) {
	...
	contact.save(errorHandler.wrap(res, function() {
		res.jsonp(contact);
	}));
};
